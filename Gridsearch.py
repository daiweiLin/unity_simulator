"""
Created on 2019-09-25 8:40 PM
Author: Daiwei Lin

Grid search to find maximum possible rewards of PLA
"""
...
from LASAgent.LASGridSearchAgent import *
...
def init(mode, algorithm, num_visitors, unity_dir, save_dir, no_graphics=False, interact_with_app=True):
    ...
    if mode == 'PLA':
        ...
        if algorithm == 'gridsearch':
            agent = LASGridSearchAgent('Baseline_Agent', observation_dim=24, action_dim=11, num_observation=25,
...
def run(mode, algorithm, behaviour, agent, visitors_behaviour):
    ...
    if mode == 'PLA':
        # if algorithm == 'gridsearch':
        total_steps = agent.gridsearchagent.nb_epochs * agent.gridsearchagent.nb_epoch_cycles * \
                      agent.gridsearchagent.nb_rollout_steps
        s = 1
        while s <= total_steps:

            take_para_action_flag, para_action, env_reset = agent.feed_observation(observation)

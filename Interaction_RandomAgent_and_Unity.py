...
def init(mode, num_visitors, unity_dir, no_graphics=False, interact_with_app=True, save_dir=None):
    ...
    if mode == 'PLA':
        ...
        # initialize ml agent
        agent_action_dim = 11
        return env, visitor_bh, agent_action_dim, behaviour

    elif mode == 'SARA':
        # initialize ml agent
        agent_action_dim = 168
        return env, visitor_bh, agent_action_dim, None


def run(mode, behaviour, agent_act_dim, visitors_behaviour):
    ...
    if mode == 'PLA':
        ...
        while s <= total_steps - 1:
            ...
            take_para_action_flag, para_action = np.random.randn(agent_act_dim).tolist()
...
if __name__ == '__main__':
    ...
    env, visitors_bh, agent_act_dim, bh = init(mode=learning_mode, num_visitors=n_visitors,
                                       unity_dir=unity_dir, no_graphics=no_graphics,
                                       interact_with_app=interact_with_app,
                                       save_dir=save_dir)
    run(mode=learning_mode, behaviour=bh, agent_act_dim=agent_act_dim, visitors_behaviour=visitors_bh)

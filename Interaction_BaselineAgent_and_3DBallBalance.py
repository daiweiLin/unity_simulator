...
def init(mode, algorithm, num_visitors, v_epsilon, unity_dir, save_dir, no_graphics=False, interact_with_app=True):
    ...
    if mode == 'SARA':
        # initialize ml agent
        if algorithm == 'ddpg':
            agent = SpinUpDDPGAgent('SpinUp_DDPG_Agent', observation_dim=8, action_dim=2,
                                     env=env, env_type='Unity', save_dir=save_dir)
        elif algorithm == 'ppo':
            agent = SpinUpPPOAgent('SpinUP_PPO_Agent', observation_dim=8, action_dim=2,
                                  env=env, env_type='Unity', save_dir=save_dir)
        elif algorithm == 'td3':
...
def run(mode, algorithm, behaviour, agent, visitors_behaviour):
    ...
    if mode == 'SARA':
        if algorithm == 'ddpg':
            ...
            LAS_action = agent.action_space.sample().tolist()
        elif algorithm == 'ppo':
            total_steps = agent.local_steps_per_epoch * agent.epochs
            LAS_action = agent.action_space.sample().tolist()
        elif algorithm == 'td3':
            total_steps = agent.td3_agent.total_steps
        ...
        while s <= total_steps - 1:
            ...
            new_LAS_action = agent.interact(observation, reward, d=done)

            LAS_action = new_LAS_action.tolist()
            s += 1

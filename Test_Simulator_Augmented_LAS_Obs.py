...
def init(mode, algorithm, num_visitors, unity_dir, save_dir, no_graphics=False, interact_with_app=True):
    ...
    if mode == 'PLA':
        ...
        if algorithm == 'ddpg':
            ...
            agent = LASSpinUpTD3Agent('SpinUP_TD3_Agent', observation_dim=34, action_dim=11, num_observation=25,
                                  env=env, env_type='Unity', load_pretrained_agent_flag=False, save_dir=save_dir)
        ...
        if algorithm == 'ddpg':
            ...
            agent = LASSpinUpTD3Agent('SpinUP_TD3_Agent', observation_dim=34, action_dim=168, num_observation=25,
                                      env=env, env_type='Unity', load_pretrained_agent_flag=False, save_dir=save_dir)
        return env, visitor_bh, agent, None
    elif mode == 'Random':
        # initialize random action agent
        agent = LASRandomAgent('RandomAgent', observation_dim=34, action_dim=168, num_observation=25, env=env,
